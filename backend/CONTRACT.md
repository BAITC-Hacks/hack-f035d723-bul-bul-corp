# API 1.2: контракт frontend / QA

Этот файл и `openapi.json` фиксируют реализованный контракт backend. Не является подтверждением, что другие участники уже приняли изменения. Аккаунты и профили описаны в [ACCOUNTS.md](ACCOUNTS.md); порядок финальной интеграции в [HANDOFF.md](HANDOFF.md).

## Изменения для интеграции

- Обычная авторизация: HttpOnly-cookie sana_session, credentials: include, X-CSRF-Token на изменяющих запросах. Регистрация, вход, сессия, выход и профили реализованы. Подробности в ACCOUNTS.md.
- X-Demo-Identity действует только при DEMO_MODE=true. По умолчанию демовход отключён. Владелец задачи и автор отклика определяются сервером по профилю сессии.
...
Адрес при локальном запуске: `http://localhost:8000`. UTF-8 JSON. Все бизнес-маршруты требуют сессию; изменяющие запросы также требуют X-CSRF-Token. В явно включённом деморежиме допускается X-Demo-Identity из /api/demo-identities. /api/health, /docs и /openapi.json доступны без входа; /api/demo-identities открыт только при DEMO_MODE=true. Регистрация и вход не требуют прежней сессии, но проверяют Origin и частоту запросов.

Бизнес-маршруты возвращают HTTP 200. Регистрация возвращает 201, выход и смена пароля 204. Запросы с телом требуют JSON-объект. Неизвестные ключи и `null` вместо строк дают 422. Опущенные необязательные поля используют значения по умолчанию. Точное определение каждого объекта, nullable-поля и ответы маршрутов находятся в `openapi.json` и `/docs`.

## Идентичности

Ниже описан совместимый деморежим. Для настоящих аккаунтов используется session.user.profile; owner_id и team_id указывают на профиль, не на личный аккаунт.
...
`/questions` определяет недостающие сведения по тем же правилам, что рейтинг, независимо от подтверждения карточки. Непустое «не знаю», неизмеримый критерий или нераспознаваемый контакт требуют уточнения. Данные, пользователи, результат и критерии приоритетны; вопросы о названии/теме откладываются, пока есть недостающие содержательные поля. Это правило действует для локального режима и допустимого выбора внешней модели. Формат ответа и число вопросов (3–5) не изменены.
...
`answers` необязателен и равен `[]`; `question` обязателен, непустой после trim; `answer` по умолчанию `""`. Исходное описание берётся из текущих полей задачи. Поле `consultation` объединяет консультации и порядок обратной связи. Вопросы не меняют карточку. Compose сохраняет изменённый черновик, снимает подтверждение и не публикует. Если поля не изменились, включая пустые ответы и повтор уже сохранённых ответов, состояние, рейтинг, version и updated_at сохраняются. Пользователь может править любой результат через PATCH. Ответы на предлагаемые уточнения добавляются целиком к соответствующему полю через перевод строки, не заменяя прежние сведения. Повтор того же ответа не добавляет дубликат. Это действует и при fallback; AI не может удалить прежний текст или известное уточнение. Если непустой ответ не удалось перенести в карточку целиком, /compose возвращает 422 с code=unassigned_answers и field_errors по ключам answers.<индекс>.question. Весь запрос отклоняется без частичного сохранения, прежнее подтверждение сохраняется. Используйте вопрос из /questions либо внесите ответ в нужное поле через PATCH. Заголовок X-AI-Source присутствует и в этой ошибке.
...
Для бизнес-показателей вне исходного словаря принимаются «действие + показатель + изменение/цель с единицей» и «показатель + явная граница с единицей»: например, `Снизить дефицит на 20%`, `Сократить потребление воды на 100 литров`, `Доля возвратов не более 2%`. Единицы включают проценты, время, валюту, массу, объём и кВт·ч; предусмотрены русские/английские формулировки. Название показателя обязательно, даты/версии/идентификаторы вместо показателя не принимаются. Это расширение правил оценки, не изменение структуры API или веса категории (15 баллов).
